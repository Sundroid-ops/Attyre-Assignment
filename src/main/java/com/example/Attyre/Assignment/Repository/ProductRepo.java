...
public interface ProductRepo extends JpaRepository<Product, Long> {
    @Query("SELECT p FROM Product p " +
        "WHERE p.category IN ?1 " +
            ...
            "AND (p.style IS NULL OR p.style IN ?4)")
    List<Product> getProductsByUserPreference(Set<String> categories, Set<String> brands, Set<Season> seasons, Set<String> styles);
}
